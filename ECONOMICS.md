...
> - **DR-29 — reward distribution deferred to M5; v1 weight source = plain Lock on the light Kupo/Ogmios path.** The yield-bearing "lock-that-delegates" hybrid and any reward routing are **not** v1. v1 reads exact locked lovelace from a **plain Lock** on the light path. This sharpens §5/§8/§9.
...
```
  ...
  ┌──────────────┐              ┌────────────────┐             ┌──────────────────────────────┐
  │ user LOCKS   │   Kupo/      │ sum unspent    │  set_stake  │ pallet-talk-stake            │
  │ N ADA at     │──Ogmios────▶ │ coins per      │──(operator──▶│ AllowedStake: addr -> weight │
...
| | **Delegate** (epoch_stake) | **Lock** (Plutus + Kupo) | **Lock-that-delegates** (hybrid) |
|---|---|---|---|
| **What's read** | Active stake delegated to the Cogno pool, from db-sync `epoch_stake` | Exact locked lovelace at the lock validator (the merged `talk_vault`, DR-18), via Kupo `?unspent` | Same as Lock, but the lock address also delegates |
...
| **Infra** | **db-sync** (cardano-node + Postgres, ~24GB RAM, 700GB+ SSD) — **breaks PLAN.md light path** | **Kupo + Ogmios only** (PLAN.md's stance, already needed for M2b) | **Kupo + Ogmios only** |
...
- **Delegate: do NOT use as the primary meter.** Its ~10-day activation lag and 5-day quantization make it a poor rate-limiter, and it forces db-sync, breaking the documented light path. Use it *at most* as a slow-moving, coarse **loyalty/bonus multiplier** layered on top of the lock weight (it also has the nice property of driving the project's own SPO stake). The `pallet-talk-stake` map and the bucket are identical for both sources, so they compose: `stake_weight = f(lock) + small_delegation_bonus`.
...
- **Reorg correctness is the follower's job.** Never grant weight off a 0-confirmation lock. Kupo surfaces rollbacks only via the HTTP `ETag` (latest block header hash) and checkpoints, so the follower must filter matches by `created_at`/`spent_at` slot past reorg depth and compare ETags before submitting `set_stake`. Get this wrong and a rolled-back lock mints phantom capacity.
...
- **§2 (stack diagram):** Add `pallet-talk-stake` and `pallet-talk-capacity` (or fold capacity into `pallet-microblog`) to the runtime box. The follower now also reads **amounts** (locked lovelace), not just signatures — annotate the Cogno-Follower box: "reads CIP-8 identity **and** talk-stake lock amounts → `set_stake`." Keep "Kupo/Ogmios, **NOT** db-sync" (Lock path preserves it).
- **§3 (approaches table):** Under Approach A, note the gate is now **metered** (regenerating capacity), and that the Lock path keeps the light Kupo/Ogmios infra (only the *rejected* Delegate path would force db-sync).
- **§4 (end-to-end):** 
...
5. **Weight source for launch. — DECIDED: plain Lock on the light Kupo/Ogmios path; hybrid + rewards deferred to M5 (DR-29, DR-33).** v1 reads exact locked lovelace from a **plain Lock** (no delegation) on the light path; the default posting-power read is L3 `AllowedStake`, with Kupo as an optional cross-check and **no Blockfrost**. The yield-bearing "lock-that-delegates" hybrid and any reward routing are deferred to M5. Delegate-alone stays rejected (~10-day lag + db-sync). *(Original question: Lock vs hybrid vs Delegate-bonus.)*
