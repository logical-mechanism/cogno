...
| **Per-validator Cardano infra** | Operator only (Kupo/Ogmios) | **Every** block producer: cardano-node + db-sync + Postgres (~24GB RAM, 700GB+ SSD) | Operator only |
...
│  │                               #   OPTIONAL Kupo/Ogmios vault/thread-ownership check -> link_identity/revoke
...
└─ infra/
   └─ devnet/
      ├─ docker-compose.yml         # cardano-node(PREPROD; DR-31) + Ogmios + Kupo + [Postgres for indexer]
      ├─ .env.example
...
docker compose -f infra/devnet/docker-compose.yml up -d    # cardano-node(preprod) + Ogmios + Kupo
...
- **Operational burden, even on the light path.** The operator still runs `cardano-node` + Kupo + Ogmios. Initial `cardano-node` sync is multi-day on mainnet (use **preprod** for the devnet, DR-31). Approach B would impose `cardano-node + db-sync + Postgres` (~24GB RAM, 700GB+ high-IOPS SSD) on **every** block producer — the reason we are not recommending it for the showcase.
