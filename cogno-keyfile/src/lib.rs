...
fn secret_suri_from_envelope(env: &KeyEnvelope) -> anyhow::Result<(Scheme, String)> {
	let scheme = Scheme::parse(&env.scheme)?;
	let hexstr = env.secret_hex.strip_prefix("0x").unwrap_or(&env.secret_hex);
	let seed = hex::decode(hexstr).map_err(|e| anyhow::anyhow!("secretHex is not valid hex: {e}"))?;
...
pub fn write_envelope(path: &Path, env: &KeyEnvelope) -> anyhow::Result<()> {
	...
	#[cfg(unix)]
	{
		...
		std::fs::set_permissions(path, std::fs::Permissions::from_mode(0o600))
			.map_err(|e| anyhow::anyhow!("cannot chmod 600 {}: {e}", path.display()))?;
	}
	Ok(())
}
...
mod tests {
	...
	fn prod_refuses_dev_key() {
		...
		assert!(assert_not_dev_key(&alice, true).is_err(), "prod must refuse //Alice");
		assert!(assert_not_dev_key(&alice, false).is_ok(), "non-prod allows dev key");
		let (fresh, _) = generate(Scheme::Sr25519, "real").unwrap();
		assert!(assert_not_dev_key(&fresh, true).is_ok(), "prod allows a fresh key");
	}
}
