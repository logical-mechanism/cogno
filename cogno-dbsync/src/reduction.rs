...
mod tests {
	...
	fn rust_matches_js_observation_equivalence_fixture() {
		use codec::Encode;
		let path = concat!(env!("CARGO_MANIFEST_DIR"), "/src/fixtures/observation-equivalence.json");
		...
		for case in cases {
			...
			for ((beacon, lovelace), exp) in entries.iter().zip(expected_entries) {
				...
				assert_eq!(hex_encode(beacon), exp_hex, "case {name}: beacon diverges from the golden");
				assert_eq!(*lovelace, exp_lovelace, "case {name}: lovelace diverges from the golden");
			}
