...
|---|---|
| Rust | **1.90.0** (pinned in [`rust-toolchain.toml`](rust-toolchain.toml) — installed automatically by rustup). Do **not** bump it: stable ≥ ~1.91 breaks the `sp_io` wasm link. |
...
- **Spec-bump discipline.** Bump `spec_version` (currently **200**) *only* for encoding-affecting
