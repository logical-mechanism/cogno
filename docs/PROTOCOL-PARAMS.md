...
it. This is a snapshot of **spec_version 222**.
...
| **spec_version** | **222** | `VERSION` — `runtime/src/lib.rs` |
| transaction_version | 8 | `VERSION` — `runtime/src/lib.rs` |
| `DESCRIPTOR_SPEC_VERSION` (frontend lockstep) | 222 — must equal `spec_version`; `npm run lint` fails on drift, and a mismatch blocks posting | `DESCRIPTOR_SPEC_VERSION` — `app/src/lib/chain/client.ts` |
