...
> and its framing is stale: it references the retired `L2-follower` design and a *shadow / default-off*
> enforcement mode. In the restart the `cardano-observer` inherent is the **sole** weight writer and
> **enforces from genesis**, and the reduction lives in the Rust `cogno-dbsync` crate. Current overview:
