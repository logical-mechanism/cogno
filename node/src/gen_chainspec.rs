...
pub fn run(cmd: &GenChainSpecCmd) -> Result<(), String> {
	...
	let committee: Vec<AccountId> = if cmd.committee_keys.is_empty() {
		...
		for p in &cmd.committee_keys {
			...
			s.require_sr25519_public().map_err(|e| format!("committee key {}: {e}", p.display()))?;
			v.push(s.account_id());
		}
		v
	};
	...
	for e in &cmd.endow {
		extra.push(
			AccountId32::from_ss58check(e.trim()).map_err(|err| format!("--endow {e:?}: {err:?}"))?,
		);
	}
	...
	for b in &cmd.bootnodes {
		boot_nodes
			.push(b.parse::<MultiaddrWithPeerId>().map_err(|err| format!("--bootnode {b:?}: {err}"))?);
	}
	...
	let name = cmd
		.name
		.clone()
		.unwrap_or_else(|| if dev_base { "Cogno Dev (operator)".into() } else { "Cogno Preprod (operator)".into() });
	let id = cmd
		.id
		.clone()
		.unwrap_or_else(|| if dev_base { "cogno-dev-operator".into() } else { "cogno-preprod-operator".into() });
	let chain_type = if dev_base { ChainType::Development } else { ChainType::Live };
	...
	eprintln!("    raw:   {}   ← run the node with `--chain` this", cmd.out_raw.display());
