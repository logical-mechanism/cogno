...
impl InherentDigest for CardanoObsInherentDigest {
	...
	fn from_inherent_data(
		...
		match inherent_data.get_data::<CardanoObservation>(&INHERENT_IDENTIFIER) {
			Ok(Some(obs)) => {
				Ok(vec![DigestItem::PreRuntime(CARDANO_OBS_DIGEST_ID, obs.reference.encode())])
			},
			Ok(None) => Ok(vec![]),
	...
	fn value_from_digest(
		...
		if count > 1 {
			return Err(format!("header carries {count} cobs PreRuntime digests (expected at most 1)").into());
		}
		Ok(found.map(SealedAnchor::Sealed).unwrap_or(SealedAnchor::NoSeal))
	}
}
...
mod tests {
	...
	fn cref(slot: u64, hash: u8) -> CardanoRef {
		CardanoRef { slot, block_hash: [hash; 32] }
	}
	...
	fn from_inherent_data_seals_the_reference_and_round_trips() {
		...
		id.put_data(INHERENT_IDENTIFIER, &obs(reference.clone())).unwrap();
		...
		assert!(matches!(&digests[0], DigestItem::PreRuntime(id, _) if id == &CARDANO_OBS_DIGEST_ID));
	...
	fn value_from_digest_missing_item_is_noseal_not_error() {
		...
		assert_eq!(CardanoObsInherentDigest::value_from_digest(&[]).unwrap(), SealedAnchor::NoSeal);
	}
