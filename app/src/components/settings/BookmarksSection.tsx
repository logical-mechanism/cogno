...
export function BookmarksSection() {
  ...
  const count = useBookmarkList().length;

  return (
    <EmptyState
      title={count > 0 ? `${count} bookmarked ${count === 1 ? "post" : "posts"}` : "No bookmarks yet"}
      description="Saved posts are kept on this device only — never written to the chain or synced across devices. Bookmark a post from the ··· menu on it."
