...
// components/icons — the inline-SVG icon set for the kit.
//
// Artwork: Material Design Icons (Google), Apache-2.0 — https://github.com/google/material-design-icons
// Every path below is copied verbatim from that repo's `src/<category>/<name>/materialicons/24px.svg`
// (or `materialiconsoutlined/` for the outlined variants); the Material name is on each component.
// Attribution lives in NOTICE at the repo root. The Spinner geometry (circle + quarter arc) is ours.
//
// Every icon is a tiny presentational SVG: 24px viewBox, `fill="currentColor"` (so colour comes from
// the consuming element's `color`/token), no external resource, no state. The like/nav icons that have
// an active state ship an `outline` (default) + `filled` variant via a `filled?: boolean` prop.
// `cg-spin` (the Spinner) lives here too.
//
...
export interface IconProps extends SVGProps<SVGSVGElement> {
  /** Filled variant for stateful icons (like/nav). Ignored by single-variant icons. */
  filled?: boolean;
...
export function IconLike(p: IconProps) {
  // Material "favorite" / "favorite_border".
  if (p.filled) {
    return (
      <svg {...svgProps(p)}>
...
export function IconDownvote(p: IconProps) {
  // Material "arrow_circle_down" / "arrow_downward". Call sites rotate the default 180° to get the
  // upvote arrow, so the outline variant has to be a bare, vertically asymmetric arrow.
  if (p.filled) {
    return (
      <svg {...svgProps(p)}>
...
export function IconBell(p: IconProps) {
  // Material "notifications" / "notifications_none".
  if (p.filled) {
    return (
      <svg {...svgProps(p)}>
...
export function IconHome(p: IconProps) {
  // Material "home" (filled + outlined variants of the same glyph).
  if (p.filled) {
    return (
      <svg {...svgProps(p)}>
...
export function IconProfile(p: IconProps) {
  // Material "person" / "person_outline".
  if (p.filled) {
    return (
      <svg {...svgProps(p)}>
...
export function IconEye(p: IconProps) {
  // Material "visibility" — the click-to-reveal cover affordance for gated images.
  return (
    <svg {...svgProps(p)}>
...
export function IconEyeOff(p: IconProps) {
  // Material "visibility_off" — re-cover a revealed image (put the reveal gate back).
  return (
    <svg {...svgProps(p)}>
