...
// models) — which sidesteps the KupoProvider.fetchCostModels gap that the off-chain scripts
// work around with an explicit setCostModels(). MeshJS is browser-only, so it is imported
// dynamically and this module stays import-safe during the static export. The `import type` is
// fully erased at build time, so it never pulls the runtime bundle into SSG.
//
