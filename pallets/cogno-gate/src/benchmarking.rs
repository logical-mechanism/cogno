...
const KEY_HEX: &str = "a401010327200621582073fea80d424276ad0978d4fe5310e8bc2d485f5f6bb3bf87612989f112ad5a7d";
...
fn hx(s: &str) -> alloc::vec::Vec<u8> {
	(0..s.len() / 2).map(|i| u8::from_str_radix(&s[2 * i..2 * i + 2], 16).expect("valid hex")).collect()
}
...
mod benchmarks {
	...
	fn link_identity_signed() -> Result<(), BenchmarkError> {
		...
		let cose_sign1: BoundedVec<u8, ConstU32<512>> =
