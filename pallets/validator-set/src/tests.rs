...
fn new_session_drains_offline_validators() {
    ...
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            published,
            Some(vec![1, 2]),
            "the offline validator is drained from the set"
        );
        assert_eq!(Validators::<Test>::get().to_vec(), vec![1, 2]);
        assert!(
...
fn new_session_drains_multiple_offline_validators() {
    ...
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            published,
            Some(vec![1]),
            "both offline validators are drained"
        );
...
fn add_then_mark_offline_then_session_drains_the_added_validator() {
    ...
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            published,
            Some(vec![1, 2, 3]),
            "the offline-flagged addition is drained"
        );
        assert_eq!(Validators::<Test>::get().to_vec(), vec![1, 2, 3]);
        assert!(
            crate::OfflineValidators::<Test>::get().is_empty(),
...
fn mark_for_removal_silently_drops_when_offline_queue_is_full() {
    ...
    new_test_ext().execute_with(|| {
        ...
        let offence = MockOffence {
            offenders: vec![(6u64, 6u64)],
        };
        assert_ok!(<ValidatorSet as ReportOffence<_, _, _>>::report_offence(
            vec![],
            offence
        ));
        ...
        assert!(
            !queue.contains(&6u64),
            "the overflowing id was dropped, not queued"
        );
    });
}
