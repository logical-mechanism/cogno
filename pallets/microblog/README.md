...
signal; the target must be identity-bound and self-votes are rejected; call indices 11 & 12), `repost`,
...
Storage version **4** (migrated v1→v4). Posts, per-author indexes, post votes (with a reverse
liked-posts index), account-reputation votes (`AccountVotes` / `AccountVoteTally`, keyed by target
account), polls, reposts, follows, and a top-level-post index — every collection is
`BoundedVec`-bounded (a full index returns an error, never a silent drop). `Post` carries
`{ author, text, parent, quote, at }`.
