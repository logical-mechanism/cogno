...
impl<T: Config> UncheckedOnRuntimeUpgrade for InnerMigrateV0ToV1<T> {
    ...
    fn post_upgrade(state: Vec<u8>) -> Result<(), sp_runtime::TryRuntimeError> {
        let before: u64 = Decode::decode(&mut &state[..]).map_err(|_| {
            sp_runtime::TryRuntimeError::Other("microblog v1: bad pre_upgrade state")
        })?;
        let after = Posts::<T>::iter().count() as u64;
        ensure!(
            before == after,
            "microblog v1: Posts count changed during migration"
        );
        ensure!(
