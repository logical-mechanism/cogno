...
//!
//! `legacy` holds RETIRED on-chain shapes that more than one migration has to address (today: the
//! v4..=v9 per-author blob, written by `v4` and retired by `v10`). A shape lives with the era it
//! belongs to, not with whichever migration ends its life — read the alias-naming warning there before
//! adding one.
//!
