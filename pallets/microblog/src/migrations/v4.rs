...
impl<T: Config> UncheckedOnRuntimeUpgrade for InnerMigrateV3ToV4<T> {
    ...
    fn post_upgrade(state: Vec<u8>) -> Result<(), sp_runtime::TryRuntimeError> {
        let expected: u64 = Decode::decode(&mut &state[..]).map_err(|_| {
            sp_runtime::TryRuntimeError::Other("microblog v4: bad pre_upgrade state")
        })?;
        ...
        let by_author: u64 = TopLevelByAuthor::<T>::iter()
        ...
        let mut ids: Vec<u64> = Posts::<T>::iter()
            .filter(|(_, p)| p.parent.is_none())
            .map(|(id, _)| id)
            .collect();
        ids.sort_unstable();
