...
impl<T: Config> UncheckedOnRuntimeUpgrade for InnerMigrateV1ToV2<T> {
	...
	fn pre_upgrade() -> Result<Vec<u8>, sp_runtime::TryRuntimeError> {
		...
		let likes = Votes::<T>::iter().filter(|(_, _, r)| matches!(r.dir, VoteDir::Up)).count() as u64;
		Ok((following, likes).encode())
	}

	#[cfg(feature = "try-runtime")]
	fn post_upgrade(state: Vec<u8>) -> Result<(), sp_runtime::TryRuntimeError> {
		let (following, likes): (u64, u64) = Decode::decode(&mut &state[..])
			.map_err(|_| sp_runtime::TryRuntimeError::Other("microblog v2: bad pre_upgrade state"))?;
		ensure!(
