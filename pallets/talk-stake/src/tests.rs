    ...
    new_test_ext().execute_with(|| {
        ...
        for (i, seq) in sequences.iter().enumerate() {
            ...
            if seq.iter().filter(|&&w| w > 0).count() >= 2 {
                assert!(
                    last < running_sum,
                    "stored weight must never be the sum of observations"
                );
            }
        }
    });
...
fn every_apply_weight_emits_its_own_event() {
    new_test_ext().execute_with(|| {
        ...
        for w in [
            10_000_000u128,
            25_000_000,
            25_000_000, /* idempotent */
            0,          /* unlock */
        ] {
            System::reset_events();
            Pallet::<Test>::apply_weight(&ALICE, w);
            assert_eq!(
                stake_set_events_for(ALICE),
                1,
                "each accepted write must emit once"
            );
            System::assert_last_event(
                Event::StakeSet {
                    who: ALICE,
...
fn explicit_zero_matches_unbound_zero() {
    new_test_ext().execute_with(|| {
        ...
        Pallet::<Test>::apply_weight(&ALICE, 0);
        assert_eq!(
            AllowedStake::<Test>::get(ALICE),
            AllowedStake::<Test>::get(BOB)
        );
        assert_eq!(AllowedStake::<Test>::get(ALICE), 0);
        System::assert_last_event(
            Event::StakeSet {
...
fn apply_voting_power_overwrites_and_is_independent_of_allowed_stake() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            AllowedStake::<Test>::get(ALICE),
            50_000_000,
            "stake untouched by voting-power set"
        );
        assert_eq!(VotingPower::<Test>::get(ALICE), 800_000_000);
        Pallet::<Test>::apply_weight(&ALICE, 0);
        assert_eq!(
            VotingPower::<Test>::get(ALICE),
            800_000_000,
            "voting power untouched by stake unlock"
        );
    });
}
