...
pub async fn committee_members(rpc: &Rpc) -> anyhow::Result<Vec<AccountId32>> {
	...
	Ok(rpc.storage_decode::<Vec<AccountId32>>(&key, None).await?.unwrap_or_default())
}
...
pub async fn proposal_hashes(rpc: &Rpc) -> anyhow::Result<Vec<H256>> {
	...
	Ok(rpc.storage_decode::<Vec<H256>>(&key, None).await?.unwrap_or_default())
}
...
pub async fn proposal_of(rpc: &Rpc, hash: &H256) -> anyhow::Result<Option<RuntimeCall>> {
	rpc.storage_decode::<RuntimeCall>(&committee_map_key("ProposalOf", hash), None).await
}
...
pub async fn voting_of(rpc: &Rpc, hash: &H256) -> anyhow::Result<Option<VotesView>> {
	rpc.storage_decode::<VotesView>(&committee_map_key("Voting", hash), None).await
}
...
pub fn describe_call(call: &RuntimeCall) -> String {
	...
	match call {
		...
		RuntimeCall::ValidatorSet(pallet_validator_set::Call::remove_validator { validator_id }) => {
		...
		RuntimeCall::CardanoObserver(pallet_cardano_observer::Call::set_enforcement { enabled }) => {
...
pub async fn events_at(rpc: &Rpc, at: H256) -> anyhow::Result<Vec<RuntimeEvent>> {
	Ok(events_with_phase_at(rpc, at).await?.into_iter().map(|(_, e)| e).collect())
}
...
pub async fn events_with_phase_at(
	...
	let recs: Vec<EventRecord<RuntimeEvent, H256>> =
		rpc.storage_decode(&key, Some(at)).await?.unwrap_or_default();
...
pub async fn resolve_committee(
	...
	Ok(Resolved { threshold, onchain_count: onchain.len(), eligible })
}
...
fn decode_module_error(index: u8, error: &[u8; 4]) -> Option<String> {
	...
	let name = match index {
		0 => format!("System.{:?}", frame_system::Error::<Runtime>::decode(&mut input).ok()?),
		8 => format!("CognoGate.{:?}", pallet_cogno_gate::Error::<Runtime>::decode(&mut input).ok()?),
		13 => format!(
			"FollowerCommittee.{:?}",
		...
		15 => format!("Session.{:?}", pallet_session::Error::<Runtime>::decode(&mut input).ok()?),
		16 => format!(
			"CardanoObserver.{:?}",
...
pub fn ensure_executed(
	...
	let mut found = false;
	for ev in events {
		if let RuntimeEvent::FollowerCommittee(CommitteeEvent::Executed { proposal_hash: h, result }) =
			ev
		{
...
pub async fn assert_extrinsic_ok(
	...
	for (phase, ev) in events_with_phase_at(rpc, block).await? {
		...
		{
			if *i == idx {
				anyhow::bail!("{label}: extrinsic FAILED ({})", format_dispatch_error(dispatch_error));
			}
		}
	}
	Ok(())
...
pub async fn propose_motion(
	...
	if threshold == 1 {
		eprintln!("propose (threshold 1 — executes on propose) as {}", proposer.ss58());
	...
	eprintln!("propose motion (threshold {threshold}) as {}", proposer.ss58());
	...
	let index = proposal_index_for(&pevents, phash)
		.context("no FollowerCommittee.Proposed event for our motion (is the proposer a committee member?)")?;
	eprintln!("proposed motion #{index} ({phash:#x})");
	Ok(ProposeOutcome::Open { proposal_hash: phash, index })
}
...
pub async fn close_motion(
	...
	let inner = proposal_of(rpc, &proposal_hash).await?.with_context(|| {
		format!("close: no open proposal {proposal_hash:#x} (already closed/executed, or wrong hash?)")
	})?;
	let length_bound = tx::length_bound(&inner);
	...
	if has_approved(&cevents, proposal_hash) {
		...
		eprintln!("✓ close → Disapproved — motion removed without executing (finalized in {block:#x})");
		Ok(CloseOutcome::Disapproved)
	} else {
		anyhow::bail!("close: motion #{index} did not resolve — no Approved/Disapproved event in {block:#x}")
	}
}
...
pub async fn via_committee(
	...
	let voters: Vec<&Signer> =
		resolved.eligible.iter().take(threshold as usize).map(|&i| &signers[i]).collect();
	anyhow::ensure!(
	...
	match propose_motion(rpc, ctx, inner, proposer, threshold).await? {
		...
		ProposeOutcome::Open { proposal_hash, index } => {
...
mod tests {
	...
	fn ensure_executed_flags_inner_revert() {
		...
		let err = DispatchError::Module(ModuleError { index: 14, error: [0, 0, 0, 0], message: None });
