...
pub async fn run_prove(account: &str, nonce: Option<&str>, ws: &str) -> anyhow::Result<()> {
    ...
    println!(
        "  cogno-chain-cli identity bind --cose-sign1 0x<signature> --cose-key 0x<key> --ws {ws}"
    );
    Ok(())
}
...
pub async fn run_bind_stake(
    ...
    let xt = build_bare(call);
    eprintln!(
        "identity bind-stake (BARE/unsigned — the stake-key COSE proof is the authorization)"
    );
    let block = rpc
        .submit_and_watch(&xt, true, "identity bind-stake")
        .await?;
