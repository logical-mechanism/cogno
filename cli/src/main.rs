...
struct GovOpts {
	...
	#[arg(long = "committee-signing-key-file", value_name = "PATH", required = true)]
...
async fn run(cli: Cli) -> anyhow::Result<()> {
	match cli.cmd {
		Command::Key { cmd } => match cmd {
			KeyCmd::Gen { scheme, out, description } => cmd_key_gen(&scheme, &out, &description),
		},
		...
		Command::Validator { cmd } => match cmd {
			ValidatorCmd::Add { validator, gov } => {
				drive_governed(&gov, calls::add_validator(calls::parse_account(&validator)?)).await
			},
			ValidatorCmd::Remove { validator, gov } => {
				drive_governed(&gov, calls::remove_validator(calls::parse_account(&validator)?)).await
			},
			ValidatorCmd::SetKeys { account_key, aura_key, grandpa_key, ws, prod, genesis } => {
				cmd_set_keys(&account_key, &aura_key, &grandpa_key, &ws, prod, genesis.as_deref()).await
			},
		},
		Command::Committee { cmd } => match cmd {
			CommitteeCmd::List { ws } => cmd_committee_list(&ws).await,
			CommitteeCmd::Vote { proposal, index, reject, seat, offline, spec_version, tx_version, nonce } => {
				cmd_committee_vote(&seat, &proposal, index, reject, offline, spec_version, tx_version, nonce)
					.await
			},
			CommitteeCmd::Close { proposal, index, seat } => {
				cmd_committee_close(&seat, &proposal, index).await
			},
			CommitteeCmd::Submit { tx, ws, no_finalize } => cmd_committee_submit(&tx, &ws, no_finalize).await,
			CommitteeCmd::Members { cmd } => match cmd {
				MembersCmd::Add { member, gov } => cmd_members(&gov, MemberAction::Add(&member)).await,
				MembersCmd::Remove { member, gov } => cmd_members(&gov, MemberAction::Remove(&member)).await,
				MembersCmd::Set { members, prime, gov } => {
					cmd_members(&gov, MemberAction::Set { members: &members, prime: prime.as_deref() }).await
				},
			},
		},
		Command::Upgrade { cmd } => match cmd {
			UpgradeCmd::Authorize { wasm, code_hash, gov } => {
				cmd_authorize_upgrade(wasm.as_deref(), code_hash.as_deref(), &gov).await
			},
			UpgradeCmd::Apply { account_key, wasm, ws, prod, genesis } => {
		...
		Command::Identity { cmd } => match cmd {
			...
			IdentityCmd::Bind { cose_sign1, cose_key, thread, ws, genesis } => {
				identity::run_bind(&cose_sign1, &cose_key, thread.as_deref(), &ws, genesis.as_deref()).await
			},
			IdentityCmd::BindStake { cose_sign1, cose_key, ws, genesis } => {
...
fn cmd_key_gen(scheme: &str, out: &Path, description: &str) -> anyhow::Result<()> {
	...
	println!("wrote {} key {} to {}", scheme.as_str(), signer.ss58(), out.display());
	Ok(())
}
...
pub(crate) async fn connect_and_ctx(
	...
	Ok((rpc, ChainCtx { genesis: chain_genesis, spec_version, tx_version }))
}
...
async fn drive_governed(gov: &GovOpts, inner: cogno_chain_runtime::RuntimeCall) -> anyhow::Result<()> {
...
async fn propose_one_seat(
	...
	eprintln!("committee: {} member(s); motion threshold {thr}", members.len());
	match committee::propose_motion(rpc, ctx, inner, signer, thr).await? {
		...
		committee::ProposeOutcome::Open { proposal_hash, index } => {
...
enum MemberAction<'a> {
	...
	Set { members: &'a str, prime: Option<&'a str> },
}
...
async fn cmd_members(gov: &GovOpts, action: MemberAction<'_>) -> anyhow::Result<()> {
	...
	let (new_members, prime) = match action {
		MemberAction::Add(m) => {
			...
			anyhow::ensure!(!members.contains(&target), "{m} is already a committee member.");
			members.push(target);
			(members, None)
		},
		...
		MemberAction::Set { members: list, prime } => {
			let new = list
				.split(',')
				.map(|s| s.trim())
				.filter(|s| !s.is_empty())
...
async fn cmd_committee_vote(
	...
	println!("✓ vote ({}) recorded on motion #{index}", if approve { "aye" } else { "nay" });
	Ok(())
}
...
async fn cmd_committee_list(ws: &str) -> anyhow::Result<()> {
	...
	let min = if members.is_empty() { 0 } else { committee::threshold_for(members.len()) };
	if hashes.is_empty() {
		println!("no open committee motions ({} member(s), 3/5 threshold {min})", members.len());
		return Ok(());
	}
	println!(
	...
	for h in &hashes {
		match (committee::voting_of(&rpc, h).await?, committee::proposal_of(&rpc, h).await?) {
			(Some(v), Some(c)) => {
...
async fn cmd_authorize_upgrade(
	...
	let hash = match (wasm, code_hash) {
		(Some(path), None) => {
			let code =
				std::fs::read(path).with_context(|| format!("reading runtime WASM {}", path.display()))?;
...
async fn cmd_apply_upgrade(
	...
	let code = std::fs::read(wasm).with_context(|| format!("reading runtime WASM {}", wasm.display()))?;
	...
	let xt = build_signed(calls::apply_authorized_upgrade(code.clone()), &account, nonce, 0, &ctx);
	eprintln!("upgrade apply: uploading {} bytes (code_hash {hash:#x}) as {}", code.len(), account.ss58());
	let block = rpc.submit_and_watch(&xt, true, "apply_authorized_upgrade").await?;
	...
	let enacted = events.iter().any(|ev| {
		matches!(ev, cogno_chain_runtime::RuntimeEvent::System(frame_system::Event::CodeUpdated))
	});
	anyhow::ensure!(
		enacted,
...
async fn cmd_set_keys(
	...
	eprintln!("set_keys as {} (aura {} / grandpa {})", account.ss58(), aura.ss58(), grandpa.ss58());
